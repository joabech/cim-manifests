...
Target: `optee-qemu-v8` — an OP-TEE environment running on QEMU for ARMv8.
...
| `toolchains/rust` | rustup-managed | see `optee_rust/rust-toolchain.toml` |
...
- `toolchains/rust` is installed via rustup into `toolchains/rust/.cargo` and
  `toolchains/rust/.rustup`.
- `optee_rust/rust-toolchain.toml` pins `nightly-2024-05-15` with targets
  `aarch64-unknown-linux-gnu` and `arm-unknown-linux-gnueabihf`.
- buildroot may pull in its own Rust toolchain; conflicts arise when versions
  diverge.
- To triage: search `build.log` for `error[E` or lines containing `rustc`.
- Fix candidates: update `rust-toolchain.toml`, adjust buildroot Rust config,
  or align the cim toolchain version in `sdk.yml`.
- We have to be careful when updating Rust versions so we use `RUSTUP_HOME` and
  `CARGO_HOME` correctly. They should in most cases be set to folder under
  `toolchains/rust`.
